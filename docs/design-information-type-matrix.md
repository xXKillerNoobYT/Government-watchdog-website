...
| `NotificationResponse` | Account-workflow notifications only: access, cohort, consent, and unsubscribe events. | The default is a visibly labelled DEV sample until its endpoint lands. These rows are **never civic alerts** and cannot populate the Alerts page. |
