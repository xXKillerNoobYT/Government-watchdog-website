...
1. **Image (multi-stage Dockerfile, website repo):**
   - Stage 1 (node:22): `npm ci` → run `scripts/fetch-artifact.mjs` with `GW_BACKEND_DEPLOY_TOKEN` supplied as a **BuildKit secret mount** (`RUN --mount=type=secret,id=gw_deploy_token …` — never an `ARG`/`ENV`, so it cannot persist in a layer) → `npm run build:private-beta` (runs the §5 exposure check; this non-Sites gated host selects the authenticated client explicitly) → outputs `dist/` + staged `service/` + seedless accounts DB schema.
