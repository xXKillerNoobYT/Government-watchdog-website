...
export const MAX_REVIEWER_RECORDS = 50_000;
export const MAX_EVIDENCE_PER_RECORD = 2_000;
export const MAX_REVIEWER_JSON_DEPTH = 32;
export const MAX_REVIEWER_JSON_NODES = 2_000_000;
...
export interface ReviewerJsonTreeLimits {
  maxDepth: number;
  maxNodes: number;
}
...
/**
 * Iterative tree validation keeps direct callers bounded before recursive
 * web-safe checks. JSON.parse cannot create cycles, but the exported normalizer
 * also rejects cyclic/shared object graphs supplied by tests or other callers.
 */
export function assertBoundedReviewerJsonTree(
  value: unknown,
  limits: ReviewerJsonTreeLimits = {
    maxDepth: MAX_REVIEWER_JSON_DEPTH,
    maxNodes: MAX_REVIEWER_JSON_NODES,
  },
): void {
  const stack: JsonContainerFrame[] = [];
  const seen = new WeakSet<object>();
  let nodes = 0;
  let pending: JsonStackEntry | undefined = { value, path: 'response', depth: 0 };
  while (pending || stack.length > 0) {
    ...
    if (current.depth > limits.maxDepth) {
      fail('response', `must not exceed ${limits.maxDepth} levels`);
    }
    nodes += 1;
    if (nodes > limits.maxNodes) {
      fail('response', `must not exceed ${limits.maxNodes} JSON values`);
    }
    if (
      current.value === null
    ...
    if (Array.isArray(current.value)) {
      if (seen.has(current.value)) {
        fail('response', 'must be a JSON tree without shared references');
      }
    ...
    fail(current.path, 'must contain JSON values and plain objects only');
  }
}
...
function assertEvidence(value: unknown, recordPath: string): void {
  ...
  if (value.length > MAX_EVIDENCE_PER_RECORD) {
    fail(
      `${recordPath}.evidence`,
      `must contain at most ${MAX_EVIDENCE_PER_RECORD} entries`,
    );
  }
...
export function normalizeReviewerInternalEnvelope(body: unknown): ReadApiResponse {
  ...
  if (body.reviewer_internal_records.length > MAX_REVIEWER_RECORDS) {
    throw new ReviewerNormalizationError(
      `reviewer_internal_records must contain at most ${MAX_REVIEWER_RECORDS} entries`,
    );
  }

  assertBoundedReviewerJsonTree(body);
  assertWebSafe(body);
