...
// Exported so the tap-floor regression suite (test/gov2273-vault-link-tap-floor.test.ts)
// can assert against the exact CSS this page injects at runtime, not a re-typed copy.
...
/* GOV-2273 — source/version links are the primary touch target of the vault, so they
   must clear the shared 44px tap floor like the contract-tool buttons do, not render as
   ~21px bare text. inline-flex+align-items:center makes min-height apply to the anchor box
   (min-height is inert on an inline box). Width stays content-sized, so the horizontal
   footprint is unchanged and no new page overflow is introduced. */
