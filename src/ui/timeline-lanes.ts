...
function svgEl(tag: string, attrs: Record<string, string> = {}): SVGElement {
  ...
  for (const [key, value] of Object.entries(attrs)) {
    // C8: SVG carries the same risk — an <a href> inside SVG executes a javascript: URL
    // exactly as an HTML one does. Same refusal, same reason.
    if ((key === 'href' || key === 'xlink:href') && safeExternalHref(value) === null) {
      node.setAttribute('data-href-refused', 'unsafe-scheme');
      continue;
    }
    node.setAttribute(key, value);
