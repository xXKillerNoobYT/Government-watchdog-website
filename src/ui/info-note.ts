...
export function renderDefinedInfoNote(
  ...
  ensureInfoNoteStyle();
  ensureOutsideDismiss(documentRef);
  ...
  const positionPanel = (): void => {
    ...
    const compact = (
      windowRef.matchMedia?.('(max-width:760px)').matches
      ?? windowRef.innerWidth <= 760
    );
    ...
    const panelWidth = panelRect.width || Math.min(390, windowRef.innerWidth - 28);
    const panelHeight = panelRect.height;
    const maxLeft = Math.max(viewportMargin, windowRef.innerWidth - panelWidth - viewportMargin);
    const left = Math.min(
    ...
    const top = panelHeight > 0 && below + panelHeight > windowRef.innerHeight - viewportMargin
    ...
    panel.style.top = `${Math.round(Math.min(
      top,
      Math.max(viewportMargin, windowRef.innerHeight - panelHeight - viewportMargin),
    ))}px`;
  };
  ...
  const stopViewportTracking = (): void => {
    if (typeof windowRef.removeEventListener === 'function') {
      windowRef.removeEventListener('resize', followViewport);
      windowRef.removeEventListener('scroll', followViewport, true);
    }
  ...
  const startViewportTracking = (): void => {
    ...
    windowRef.addEventListener('resize', followViewport, { passive: true });
    windowRef.addEventListener('scroll', followViewport, { passive: true, capture: true });
    openObserver = new MutationObserver(() => {
      if (wrapper.isConnected) return;
      stopViewportTracking();
      panel.remove();
      if (pinnedInfoNotes.get(documentRef)?.wrapper === wrapper) {
        pinnedInfoNotes.delete(documentRef);
      }
    });
    openObserver.observe(documentRef.body, { childList: true, subtree: true });
  };

  const setOpen = (open: boolean, restoreFocus = false): void => {
    clearHoverClose();
    if (open) {
      if (panel.parentElement !== documentRef.body) documentRef.body.append(panel);
  ...
  trigger.addEventListener('click', () => {
    if (pinnedOpen) {
      ...
      pinnedInfoNotes.get(documentRef)?.close();
      pinnedOpen = true;
      wrapper.setAttribute('data-pinned', '');
      pinnedInfoNotes.set(documentRef, { wrapper, panel, close: closePinned });
      open();
  ...
  wrapper.addEventListener('focusout', (event) => {
    ...
    if (
      !pinnedOpen
      && (
        !(destination instanceof windowRef.Node)
