...
import {
  assertBoundedReviewerJsonTree,
  MAX_EVIDENCE_PER_RECORD,
  MAX_REVIEWER_RECORDS,
...
describe('normalizeReviewerInternalEnvelope', () => {
  ...
  it('bounds records and per-record evidence before projecting any rows', () => {
    expect(() => normalizeReviewerInternalEnvelope({
      reviewer_internal_records: new Array(MAX_REVIEWER_RECORDS + 1),
    })).toThrow(new RegExp(`at most ${MAX_REVIEWER_RECORDS}`));

    const tooManyReceipts = record();
    tooManyReceipts.evidence = Array.from(
      { length: MAX_EVIDENCE_PER_RECORD + 1 },
      () => ({ verification_status: 'source_recorded' }),
    );
    expect(() => normalizeReviewerInternalEnvelope({
      reviewer_internal_records: [tooManyReceipts],
    })).toThrow(new RegExp(`at most ${MAX_EVIDENCE_PER_RECORD}`));
  });

  it('bounds unknown JSON depth and node count before the recursive safety sweep', () => {
    expect(() => assertBoundedReviewerJsonTree(
      { one: { two: { three: true } } },
    ...
    )).toThrow(/5 JSON values/);
  });

  it('rejects cyclic or shared direct-call object graphs as non-JSON trees', () => {
    const shared = { safe: true };
    expect(() => assertBoundedReviewerJsonTree(
      { first: shared, second: shared },
    )).toThrow(/without shared references/);

    const cyclic: Record<string, unknown> = {};
    cyclic.self = cyclic;
    expect(() => assertBoundedReviewerJsonTree(cyclic))
      .toThrow(/without shared references/);
  });
