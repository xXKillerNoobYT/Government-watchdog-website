...
function downloadRelease(ref, outDir) {
  ...
  // Prefer gh (ambient auth); fall back to a token-authenticated download.
  // Either way the token is passed via env/args gh manages — never echoed.
  try {
...
function main() {
  ...
  if (kind.mode === 'local') {
    ...
    ({ tarball } = downloadRelease(ref, work));
  }
